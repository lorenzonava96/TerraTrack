...
def displacement_analysis(
    img1, img2,
    method='optical_flow',  # Options: 'optical_flow', 'block_matching'
    block_size=16,
    overlap=0.8,
    match_func='phase_cross_corr',  # Options: 'phase_cross_corr', 'ssd', 'ncc', 'autorift', or custom
    zero_mask=None,  # Optional mask for invalid areas
    filter_params=None,  # Parameters for `filter_displacements`
    plot=True,  # Option to visualize the displacement field
    arrow_scale=10
):
    """
    Wrapper function to compute subpixel displacement using block matching, 
    dense optical flow, or a combination of both methods.

    Parameters:
    - img1: First image (reference).
    - img2: Second image (target).
    - method: Displacement computation method ('optical_flow', 'block_matching', or 'combined').
    - block_size: Size of blocks for block matching (if method includes block matching).
    - overlap: Overlap percentage for block matching (0 to 1).
    - match_func: Matching function for block matching ('phase_cross_corr', 'ssd', 'ncc', 'autorift', or custom function).
    - zero_mask: Mask for invalid regions (1 for invalid, 0 for valid).
    - filter_params: Dictionary of filtering parameters for `filter_displacements`.
    - plot: Whether to plot the displacement field.
    - arrow_scale: Scale for arrows in the displacement plot.

    Returns:
    - u: Horizontal displacements (dx).
    - v: Vertical displacements (dy).
    - feature_points: Coordinates of feature points.
    - errors: Error estimates (if available).
    """
    ...
    if method == 'optical_flow':
        ...
        raise ValueError(f"Invalid method: {method}. Available options: 'optical_flow', 'block_matching', 'combined'")
    ...
    # Step 3: Plot the displacement field if requested
...
def block_matching_vectorized(img1, img2, block_size=16, overlap=0.8, match_func='phase_cross_corr'):
    """
    ...
    def fft_ncc(block1, block2):
        ...
        # Remove mean for normalized cross-correlation
        block1 -= np.mean(block1)
        block2 -= np.mean(block2)
        
        # Compute FFT of both blocks
        fft_block1 = fft2(block1)
        fft_block2 = fft2(block2)
        
        # Compute cross-power spectrum (normalized cross-correlation in frequency domain)
        cross_power = fft_block1 * np.conj(fft_block2)
        cross_corr = ifft2(cross_power / (np.abs(cross_power) + 1e-10)).real  # Avoid division by zero
        ...
        # 2) Find the second-highest peak
        #    Temporarily set the highest peak to a very small value so it won't be found again.
        cross_corr_temp = cross_corr.copy()
        cross_corr_temp[peak_idx] = -np.inf  # ensure it's not chosen again

        second_idx = np.unravel_index(np.argmax(cross_corr_temp), cross_corr_temp.shape)
        second_peak = cross_corr_temp[second_idx]

        # 3) Peak-to-peak SNR
        #    If `second_peak` is extremely small or negative, 
        #    this ratio can get very large. Adjust as needed for your application.
        snr = max_corr / (second_peak + 1e-10)

        # 4) Subpixel refinement around the primary peak using a small window
        window_size = 3
        ...
        # Translate local coordinates to global
        refined_x += x_min - cross_corr.shape[1] // 2
        refined_y += y_min - cross_corr.shape[0] // 2

        # Return subpixel displacement, primary peak, and peak-to-peak SNR
        return refined_x, refined_y, max_corr, snr

    def mean_dense_optical_flow(block1, block2):
        """
        Use dense optical flow (Farneback) to estimate displacement within a block.

        Parameters:
        - block1: Reference block (first image).
        - block2: Target block (second image).

        Returns:
        - dx: Horizontal displacement.
        - dy: Vertical displacement.
        - error: Proxy error (e.g., magnitude of displacement).
        """
        flow = cv2.calcOpticalFlowFarneback(
            block1, block2, None, 
            pyr_scale=0.5,  # Scale of the pyramid
            levels=3,       # Number of pyramid levels
            winsize=block1.shape[0],  # Window size for smoothing
            iterations=3,   # Number of iterations for each pyramid level
            poly_n=5,       # Neighborhood size
            poly_sigma=1.2, # Gaussian smoothing
            flags=0
        )
        # Average flow within the block
        dx = -np.median(flow[..., 0])
        dy = -np.median(flow[..., 1])
    ...
    if callable(match_func):  # Custom function provided
        match_func_callable = match_func
    elif match_func in match_func_map:
        match_func_callable = match_func_map[match_func]
    else:
        raise ValueError(f"Invalid match_func: {match_func}. Available options: {list(match_func_map.keys())}")

    # Calculate stride
    stride = int(block_size * (1 - overlap))
    img_shape = img1.shape

    # Generate sliding window views for blocks
...
def process_image_pairs(dat1, dat2, datax, preprocessed_stack, filter_params, zero_mask=None,
                        method='block_matching', block_size=32, overlap=0.8, match_func='phase_cross_corr', max_workers=None):
    """
    Processes image pairs to compute displacements in parallel, printing progress at intervals.
    ...
    - block_size: Block size for block matching (default: 32).
    - overlap: Overlap percentage for block matching (default: 0.8).
    - match_func: Matching function for block matching (default: 'phase_cross_corr').
    - max_workers: Maximum number of threads to use (default: all available cores).

    Returns:
    - all_results: List of results with displacements and feature points for each pair.
    """
    # Create a mapping between dates and their corresponding image indices
